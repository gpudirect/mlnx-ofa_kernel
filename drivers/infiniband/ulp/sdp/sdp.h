...
#ifndef TCPHDR_PSH
#define TCPHDR_PSH TCPCB_FLAG_PSH
#endif
...
#else
#define sdp_inet_num(sk)	(inet_sk(sk)->num)
#define sdp_inet_sport(sk)	(inet_sk(sk)->sport)
#define sdp_inet_dport(sk)	(inet_sk(sk)->dport)
#define sdp_inet_saddr(sk)	(inet_sk(sk)->saddr)
...
#define SDPSTATS_HIST(stat, size) \
	sdpstats_hist(this_cpu_ptr(&sdpstats)->stat, size, ARRAY_SIZE(this_cpu_ptr(&sdpstats)->stat) - 1, 1)
