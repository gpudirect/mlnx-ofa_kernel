...
isert_connect_request(struct rdma_cm_id *cma_id, struct rdma_cm_event *event)
{
	...
	isert_info("isert_conn: %p, initiator: %pISp\n", isert_conn,
	...
	list_add_tail(&isert_conn->node, &isert_np->pending);
...
isert_connected_handler(struct rdma_cm_id *cma_id)
{
	...
	mutex_lock(&isert_np->mutex);
	list_move_tail(&isert_conn->node, &isert_np->accepted);
	mutex_unlock(&isert_np->mutex);
		      ...
		      struct iser_rx_desc *rx_desc, unsigned char *buf)
{
	...
	if (imm_data_len != data_len) {
		...
		isert_dbg("Copying Immediate SG: %p sg_nents: %u from %p imm_data_len: %d\n",
			  cmd->se_cmd.t_data_sg, sg_nents, &rx_desc->data[0], imm_data_len);
		...
		isert_dbg("Transfer Immediate SG: %p from %p imm_data_len: %d\n",
			  &isert_cmd->sg, &rx_desc->data[0], imm_data_len);
...
isert_rx_opcode(struct isert_conn *isert_conn, struct iser_rx_desc *rx_desc,
		u32 read_stag, u64 read_va,
		u32 write_stag, u64 write_va)
{
...
isert_rx_do_work(struct iser_rx_desc *rx_desc, struct isert_conn *isert_conn)
{
	...
	u64 read_va = 0, write_va = 0;
	u32 read_stag = 0, write_stag = 0;
	int rc;
...
isert_handle_wc(struct ib_wc *wc)
{
	...
	if (likely(wc->status == IB_WC_SUCCESS)) {
		...
		if (wc->status != IB_WC_WR_FLUSH_ERR)
			isert_err("wr id %llx status %d vend_err %x isert_conn %p\n",
				  wc->wr_id, wc->status, wc->vendor_err, isert_conn);
		else
...
accept_wait:
	...
	isert_conn = list_first_entry(&isert_np->accepted,
...
/**
 *isert_put_unsol_pending_cmds() - Drop commands waiting for unsolicitate dataout
 */
static void
